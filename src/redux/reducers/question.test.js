...
describe('Question Reducer', () => {
  ...
  describe('Chosen answer', () => {
    ...
    it('When the good answer has been chosen, the question is resolved', () => {
      // given
      const previousState = {
        label: 'QUESTION', answers: []
      };

      const action = actions.CHOOSE_ANSWER.create({ correct: true });

      // when / then
      reduce(previousState, action).byUsing(reducer).expectedNextState({
        label: 'QUESTION', answers: [], resolved: true
      });
    });
  });
